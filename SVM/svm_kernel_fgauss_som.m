% SVM - Fine Gaussian Kerner
function [svm_kernel_fgauss_trained, validationAccuracy] = svm_kernel_fgauss(trainingData)
...
svm_kernel_fgauss_trained.predictFcn = @(x) svmPredictFcn(predictorExtractionFcn(x));

% Add additional fields to the result struct
svm_kernel_fgauss_trained.ClassificationSVM = classificationSVM;
svm_kernel_fgauss_trained.About = 'This struct is a trained classifier exported from Classification Learner R2016b.';
svm_kernel_fgauss_trained.HowToPredict = sprintf('To make predictions on a new predictor column matrix, X, use: \n  yfit = c.predictFcn(X) \nreplacing ''c'' with the name of the variable that is this struct, e.g. ''trainedClassifier''. \n \nX must contain exactly 41 columns because this classifier was trained using 41 predictors. \nX must contain only predictor columns in exactly the same order and format as your training \ndata. Do not include the response column or any columns you did not import into \nClassification Learner. \n \nFor more information, see <a href="matlab:helpview(fullfile(docroot, ''stats'', ''stats.map''), ''appclassification_exportmodeltoworkspace'')">How to predict using an exported model</a>.');

% Perform cross-validation
partitionedModel = crossval(svm_kernel_fgauss_trained.ClassificationSVM, 'KFold', 10);
